...
//    val res = Await.result(MandrillAsyncClient.tagDelete(MTagRequest(tag = "twotag")), DefaultConfig.defaultTimeout)
//    res.getClass shouldBe classOf[MTagResponse]
//    res.tag shouldBe "twotag"
//  }
//  it should "work getting a valid MTagResponse (blocking client)" in {
//    MandrillBlockingClient.tagDelete(MTagRequest(tag = "twotag")) match {
//      case Success(res) =>
//        res.tag shouldBe "twotag"
//      case Failure(ex) => fail(ex)
//    }
//  }
  ...
  it should "fail if the tag does not exists, with an 'Invalid_Tag_Name' code" in {
