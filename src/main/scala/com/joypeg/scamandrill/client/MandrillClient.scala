...
trait MandrillClient {
  ...
  object Endpoints extends Enumeration {
    ...
    //key
    val inbdom          = Value("inbdom",       "/key/domains.json")
    val inbadddom       = Value("inbadddom",    "/key/add-domain.json")
    val inbchkdom       = Value("inbchkdom",    "/key/check-domain.json")
    val inbdeldom       = Value("inbdeldom",    "/key/delete-domain.json")
    val inbroutes       = Value("inbroutes",    "/key/routes.json")
    val inbaddroute     = Value("inbaddroute",  "/key/add-route.json")
    val inbuproute      = Value("inbuproute",   "/key/update-route.json")
    val inbdelroute     = Value("inbdelroute",  "/key/delete-route.json")
    val inbraw          = Value("inbraw",       "/key/send-raw.json")
    //key
